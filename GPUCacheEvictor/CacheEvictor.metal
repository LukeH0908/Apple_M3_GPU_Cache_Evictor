...
kernel void in_shader_pump_probe_linear(
    // Input Buffers
    device  uint *probeBuffer [[buffer(0)]],
    device  uint *pumpBuffer [[buffer(1)]],
    ...
    // --- WORKER THREAD ---
    if (tid == 64) {
        for (uint i = 0; i < pump_element_count; i++){
            pumpBuffer[i] = 2;
        }
//        for (uint i = 0; i < 16; i++){
//            probeBuffer[i] = 2;
...
kernel void in_shader_pump_probe_linear_pc(
    // Input Buffers
    device  uint *probeBuffer [[buffer(0)]],
    device  uint *pumpBuffer [[buffer(1)]],
    ...
    if (tid == 64) {
        for (uint i = 0; i < pump_element_count; i++){
            pumpBuffer[i] = (i+16) % pump_element_count;
        }
//        for (uint i = 0; i < 16; i++){
//            probeBuffer[i] = 2;
//        }
        ...
        for (uint i = 0; i < 10; i++){
            for (uint j = 0; j < pump_element_count; j += stride_in_elements) {
                current_index = pumpBuffer[current_index]; // The core pointer-chase
                temp_pump_sum += current_index;
...
kernel void in_shader_pump_probe_random_pc(
    // Input Buffers
    device  uint *probeBuffer [[buffer(0)]],
    device  uint *pumpBuffer [[buffer(1)]],
    ...
    if (tid == 64) {
        ...
        const uint prime_stride = 7919;
        for (uint i = 0; i < pump_element_count; i++){
            pumpBuffer[i] = (i * prime_stride) % (pump_element_count);
        }
//        for (uint i = 0; i < 16; i++){
//            probeBuffer[i] = 2;
//        }
        ...
        uint random_line_index = 0;
        for (uint i = 0; i < 10; i++) {
            // This inner loop iterates from 0 to N-1
            for (uint j = 0; j < pump_element_count; j+=16) {
                
                random_line_index = pumpBuffer[random_line_index];
